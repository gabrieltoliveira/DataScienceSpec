# Assignment: Caching the Inverse of a Matrixmenos Matrix inversion is usually a
# costly computation and there may be some benefit to caching the inverse of a
# matrix rather than compute it repeatedly (there are also alternatives to
# matrix inversion that we will not discuss here). Your assignment is to write a
...
# the version of the fi.,les you want to submit, then just click the “copy to
# clipboard” button on the right hand side that should appear when you hover
# over the SHA-1 hash. Paste this SHA-1 hash into the course web site when you
# submit your assignment. If you don't want to use the most recent commit, then
# go down and find the commit you want and copy the SHA-1 hash. A valid
# submission will look something like (this is just an example!)
# 
...
# Function to define a matrix object and its following functions: set, get, setinv, getinv
...
test_matrix <- matrix(data = 1:20, nrow = 5, ncol = 4)
